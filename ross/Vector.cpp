...
//
// bool setPolar(magnitude, theta, dz)
// Last modified: 04Sep2006
//
...
// Returns:     true if successful, false otherwise
// Parameters:
//      magnitude   in      the magnitude to be set to (default 1)
//      theta       in      the angle to be set to (default 0)
//      dz          in      the z-coordinate to be set to (default 0)
//
bool Vector::setPolar(GLfloat magnitude, GLfloat theta, GLfloat dz)
{
    theta = degreesToRadians(theta);
    return set(magnitude * cos(theta), magnitude * sin(theta), dz);
...
//      magnitude   in      the magnitude to be set to (default 1)
//
bool Vector::setNorm(const GLfloat magnitude)
{
    if (!normalize()) return false;
    return set(x * magnitude, y * magnitude, z * magnitude);
...
void Vector::draw()
{
    ...
    glPushMatrix();
        ...
        // draw vector head
        GLfloat magnitude = norm();
        if ((showHead) && (magnitude >= VECTOR_HEAD_HEIGHT))
        {
            glColor3fv(color);
            glBegin(GL_TRIANGLES);
                glVertex3f(magnitude, 0.0f, 0.0f);
                glVertex3f(magnitude - VECTOR_HEAD_HEIGHT,
                           VECTOR_HEAD_WIDTH,
                           0.0f);
                glVertex3f(magnitude - VECTOR_HEAD_HEIGHT,
                           -VECTOR_HEAD_WIDTH,
...
{
    return sqrt((GLfloat)(pow(x, 2.0f) + pow(y, 2.0f) + pow(z, 2.0f)));
}   // norm()
