...
void Circle::draw()
{
    if ((color[0] == COLOR[INVISIBLE][0]) &&
        (color[1] == COLOR[INVISIBLE][1]) &&
        (color[2] == COLOR[INVISIBLE][2])) return;

	// draw vector position
    if (showPos) Vector::draw();

    // draw a polygonal approximation to the circle
    Vector edge(radius * (CIRCLE_COS_THETA - 1.0f),
                radius *  CIRCLE_SIN_THETA,
                0.0f);
    edge.setColor(color);
    edge.showHead = false;
    glPushMatrix();
	    ...
	    {
            glPushMatrix();
                glRotated((GLfloat)i * CIRCLE_THETA, 0, 0, 1);
                glTranslated(radius, 0.0f, 0.0f);
                edge.draw();
            glPopMatrix();
	    }
        glEnd();
